...
class JsonApiProcController {
  ...
  public function getCihper() {
    ...
    foreach ($proc_ids as $proc_index => $proc_id) {
      ...
      $cipher_data[$proc_index]['source_file_name'] = $proc->get('meta')
        ->getValue()[0]['source_file_name'];
      $cipher_data[$proc_index]['source_file_size'] = $proc->get('meta')
        ->getValue()[0]['source_file_size'];
      if (isset($proc->get('meta')->getValue()[0]['source_input_mode'])) {
        $cipher_data[$proc_index]['source_input_mode'] = $proc->get('meta')
          ->getValue()[0]['source_input_mode'];
      }
      $cipher_data[$proc_index]['cipher_cid'] = $proc_id;
      $cipher_data[$proc_index]['proc_owner_uid'] = $proc->get('user_id')
        ->getValue()[0]['target_id'];
      $cipher_data[$proc_index]['proc_recipients'] = $proc->get('field_recipients_set')
        ->getValue();
