        ...
        attach: function (context, settings) {
            async function encrypt() {
                let procJsLabels        = Drupal.settings.proc.proc_labels,
                    plaintext = document.getElementById('edit-plaintext').value,
                    ...
                    if (remoteKey.length > 0){
                        ...
                        const pubKeyAjax = async (remoteKeyCsv) => {
                            let response = await fetch(
                                `${window.location.origin + Drupal.settings.basePath}proc/api/getpubkey/${remoteKeyCsv}/uid`
                            );
            ...
            if (document.getElementById('edit-button')) {
                document.getElementById('edit-button').addEventListener('click', encrypt, false);
            }
        }
    };
})(jQuery);
