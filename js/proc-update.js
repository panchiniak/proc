    ...
    attach: function (context, settings) {
      ...
      $('input#edit-pass')
        ...
        .on('focusin', function () {
          ...
          if (submit.disabled == true) {
            ...
            document.querySelector('#edit-pass')
              .value = '';
          }
        });
      ...
      $('.proc-update-submit')
        .on('click', async function () {
          ...
          cipherTextsIndex.forEach(async function (item, i) {
            document.querySelector('.proc-update-submit')
              .innerText = procJsLabels.proc_button_state_processing;
            ...
            if (decrypted) {
              ...
              reader.onloadend = async function (evt) {
                if (evt.target.readyState == FileReader.DONE) {
                  ...
                  var procIDString = procID.pop()
                    .toString();
                  await (document.querySelector('[name=cipher_text_cid_' + procIDString + ']')
                    .value = ciphertext);
                  document.querySelector('[name=generation_timespan_cid_' + procIDString + ']')
                    .value = total;
                  document.querySelector('[name=browser_fingerprint_cid_' + procIDString + ']')
                    .value = BROWSER_FINGERPRINT;
                  document.querySelector('[name=generation_timestamp_cid_' + procIDString + ']')
                    .value = startSeconds;
                  if (procID.length == 0) {
                    ready = 1;
                    document.querySelector('.proc-update-submit')
                      .innerText = procJsLabels.proc_submit_saving_state;
                    // Do not submit the password:
                    var passPlaceHolder = new Array($('input[name=pass]')[0].value.length + 1)
                      .join(Math.random()
                        .toString(36)
                        .replace(/[^a-z]+/g, '')
                        .substr(0, 1));
